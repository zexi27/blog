...
import static com.zlq.blog.constant.OptTypeConst.*;
...
public class ArticleController {
    ...
    @ApiOperation(value = "从旧数据库导入数据")
    @GetMapping("/import")
    public Result<String> importArticlesFromOldDatabase(){
        Integer importCount = articleService.importArticlesFromOldDatabase();
        return Result.ok("共导入" + importCount + "篇文章");
    }

    @ApiOperation(value = "自动生成文章合适的标签")
    @PostMapping("/admin/articles/generateArticleTag")
    public Result generateArticleTag(@RequestBody ArticleVO articleVO){
        List<String> tagNameList = articleService.automaticallyGenerateTags(articleVO.getArticleTitle(),articleVO.getArticleContent());
        return Result.ok(tagNameList);
    }
