...
import com.zlq.blog.dao.*;
import com.zlq.blog.dto.*;
import com.zlq.blog.entity.Article;
import com.zlq.blog.entity.WebsiteConfig;
...
import com.zlq.blog.service.UniqueViewService;
import com.zlq.blog.util.BeanCopyUtils;
import com.zlq.blog.util.IpUtils;
import com.zlq.blog.util.UserUtils;
import com.zlq.blog.vo.BlogInfoVO;
import com.zlq.blog.vo.PageVO;
import com.zlq.blog.vo.WebsiteConfigVO;
...
public class BlogInfoServiceImpl implements BlogInfoService {
    ...
    public BlogHomeInfoDTO getBlogHomeInfo() {
        ...
        // 如果是登录用户再加上登录用户的私密文章数量
        if (UserUtils.isLogin()) {
            Integer userId = UserUtils.getLoginUser().getUserInfoId();
            Integer secretArticleCountByUserId = articleDao.selectCount(new LambdaQueryWrapper<Article>()
                    .eq(Article::getUserId, userId)
                    .eq(Article::getStatus, SECRET.getStatus())
                    .eq(Article::getIsDelete, FALSE));
            articleCount += secretArticleCountByUserId;
        }
            // 查询分类数量
        ...
        public BlogBackInfoDTO getBlogBackInfo () {
            ...
            // 查询分类数据
            List<CategoryDTO> categoryDTOList = categoryDao.listCategoryDTO(userId);
            // 查询标签数据
